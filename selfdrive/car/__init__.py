...
from enum import IntFlag, Enum, EnumMeta
...
class PlatformsType(EnumMeta):
  def __new__(metacls, cls, bases, classdict, **kwds):
    # Get member names from classdict directly
    for key, value in classdict.items():
      if isinstance(value, PlatformConfig):
        value.platform_str = key
        value.freeze()
    return super().__new__(metacls, cls, bases, classdict, **kwds)


class Platforms(str, Enum, metaclass=PlatformsType):
